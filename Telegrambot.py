...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "intro":
        ...
        "Ничего не поняли и не запомнили? Это нормально. К этим словам и тезисам мы будем возвращаться. В [табличке] (https://drive.google.com/file/d/1lMRW4SyewtrcHn6iygTodsokDa_4edgb/view?usp=drive_link) я собрала сразу ВСЕ правила чтения. Со временем теория «обрастёт фактами» и заученные правила станет естественной частью жизни. Пока мы на неё смотрим и только готовимся понять.\n\nВажно вынести все эти термины сейчас на видное место, чтобы обращаться к ним каждый раз, когда будем сталкиваться с новыми правилами чтения.",
